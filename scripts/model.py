...
def process_image(image_paths:List[str])-> List[str]: 
  """
  Function takes in the path of the image as a list of strings and converts it to RGB
...
PATH =f"{BASE_DIR}/artifacts/model_{__version__}.pt"

torch.save(model_obj.state_dict(), PATH) 
model_obj.load_state_dict(torch.load(PATH))
 
images1= process_image([f"{BASE_DIR}/artifacts/img_1.jpg"]) 
